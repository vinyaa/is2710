...
<?php
	...
	if(isset($_POST['product_categories'])){
		...
		$data3_query = "SELECT Region.region_name AS Region, SUM((Transaction.product_quantity)*(Transaction.price)) AS Revenue FROM Transaction, Region, Salesperson, Store WHERE Transaction.salesperson_id=Salesperson.emp_id AND Salesperson.store_id=Store.store_id AND Store.region_id=Region.region_id GROUP BY Region.region_name";
